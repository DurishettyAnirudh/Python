...
class VideoManagerApp(tk.Tk):
    ...
    def add_row(self, is_label_row=False, data=None):
        if is_label_row:
            ...
            # Privacy Status
            privacy_status_var = tk.StringVar()
            print(privacy_status)
            privacy_status_var.set(privacy_status)
            privacy_options = ttk.Combobox(self.frame, textvariable=privacy_status_var, values=["Public", "Private"],
                                           justify='center')
            privacy_options.grid(row=self.row_count, column=5, padx=5, pady=10, sticky="nsew")
            privacy_options.update_idletasks() 
            self.privacy_statuses.append(privacy_status)
            print(f"StringVar value: {privacy_status_var.get()}")  # This should print "Public" or "Private"
            ...
            upload_button = tk.Button(action_frame, text="Upload", command=lambda row_index=self.row_count: self.trigger_upload_app(row_index,
                                                                                                                                    title = self.title_entries[row_index-1].get(),
                                                                                                                                    category = self.category_entries[row_index-1].get(),
                                                                                                                                    video_path = self.video_paths[row_index-1].get(),
                                                                                                                                    privacy_status = self.privacy_statuses[row_index-1],
                                                                                                                                    description = self.description_entries[row_index-1].get(),
                                                                                                                                    keywords = self.keyword_entries[row_index-1].get() ))
    ...
    def trigger_upload_app(self, row_index, title,category,video_path,privacy_status ,description,keywords):
        conn = mysql.connector.connect(**DB_CONFIG)
        cursor = conn.cursor()
        cursor.execute(
        'SELECT privacy_status FROM videos where serial_number = %s',(row_index,))
        
        rows = cursor.fetchall()
        row = rows[0]
        privacy_status = row[0].lower()
        print(privacy_status)
        # for row in rows:
        #     privacy_status = row
        ...
        #for Youtube Upload
        print("python", "./Youtube/upload_video.py", video_path)
        subprocess.Popen(["python", "./Youtube/upload_video.py", "--file" , video_path, "--title", " \""+title+"\"", "--description" ," \""+ description+"\"", "--keywords" ," \""+keywords+ "\"", "--privacyStatus", privacy_status])
        print("youtube upload started")


        print(row_index)
        # For Telegram Upload
        subprocess.Popen(["python", "./telegram/telegrambot.py", str(row_index)])

        subprocess.Popen(["python", "./Email/sender.py", str(row_index)])
    ...
    def save_data(self):
        for i in range(self.row_count - 1):
            ...
            data = {
                ...
                'Privacy Status': self.privacy_statuses[i],
